...
sealed class ExtractResult {
    ...
    data class NoneFound(val reason: String) : ExtractResult()
}

/**
 * Looks for videos that a page serves openly: standard HTML5
 * <video>/<source> tags or an og:video meta tag pointing at a direct
 * file. Does NOT attempt to reverse-engineer player JS, decrypt signed
 * URLs, or use any platform-private API - that's how sites that don't
 * want to be scraped stay unscraped, by design here.
 */
class VideoLinkExtractor(private val client: OkHttpClient) {
    ...
    fun extract(pageUrl: String): ExtractResult {
        ...
        val html = fetchHtml(pageUrl) ?: return ExtractResult.NoneFound("Couldn't load the page.")

        if (DownloadPolicy.looksLikeDrmOrAuthProtected(null, html)) {
